...
class YFastTrieSL {
 private:
  ...
  struct xfast_update {
    ...
    yfast_bucket_sl* repr_to_insert = nullptr;    // The representants that have be inserted into the xfast_trie.
    yfast_bucket_sl* bucket_to_delete = nullptr;  // The buckets that have to be deleted.
  };
  ...
  //    - The node has only a right child <=> The prefix is mapped to the smallest bucket in the right subtree
  //    - The node has no children <=> The prefix is mapped to the only bucket whose representant has the same prefix as the node
  std::array<robin_hood::unordered_map<uint64_t, yfast_bucket_sl*>, t_key_width + 1> m_xfast;
  ...
  void insert_repr(yfast_bucket_sl* const b) {
    ...
    while (level > 0) {
      if (child == m_xfast[level - 1].end()) {
        if (sibling == m_xfast[level - 1].end()) {
          ...
          yfast_bucket_sl* smaller_bucket;
          yfast_bucket_sl* greater_bucket;
          yfast_bucket_sl* other_b = node->second;
  ...
  void remove_repr(uint64_t key) {
    ...
    if (sibling_left_child == m_xfast[level - 1].end() && sibling_right_child == m_xfast[level - 1].end()) {
      ...
      yfast_bucket_sl* sift_up_bucket = sibling->second;
  ...
  yfast_bucket_sl* min_repr(uint64_t level, uint64_t key) const {
    while (level > 0) {
  ...
  // that corrospondents to (level, key)
  yfast_bucket_sl* max_repr(uint64_t level, uint64_t key) const {
    while (level > 0) {
  ...
  // next greater bucket.
  yfast_bucket_sl* pred_bucket(uint64_t key) const {
    size_t first1 = find_level(key);
    yfast_bucket_sl* b = m_xfast[first1].at((key >> 1) >> (first1 - 1));
    if (b->get_repr() <= key) {
      return b;
    } else {
      return b->get_prev();
 ...
 public:
  YFastTrieSL() {
    m_xfast[t_key_width][0] = new yfast_bucket_sl(0, false, nullptr, nullptr);
  };
  // Inserts the first num keys from keys into the yfast_trie.
  YFastTrieSL(const uint64_t* keys, const size_t num) {
    m_xfast[t_key_width][0] = new yfast_bucket_sl(0, false, nullptr, nullptr);
  ...
  ~YFastTrieSL() {
    yfast_bucket_sl* b = max_repr(t_key_width, 0);
    yfast_bucket_sl* p;
    while (b != nullptr) {
      p = b->get_prev();
      delete b;
  ...
  void insert(uint64_t key) {
    ...
    yfast_bucket_sl* new_bucket = b->insert(key);
  ...
  void remove(uint64_t key) {
    --m_size;
    yfast_bucket_sl* p_bucket = pred_bucket(key);
  ...
  KeyResult<uint64_t> predecessor(uint64_t key) const {
    ...
    yfast_bucket_sl const* const search_bucket = pred_bucket(key);
  ...
  friend std::ostream& operator<<(std::ostream& o, const YFastTrieSL& yfast) {
    // General information
