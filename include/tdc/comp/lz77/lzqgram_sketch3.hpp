...
private:
    ...
    size_t m_filter_num;
    MinCountMap<qgram_t> m_filter_min;
    ...
    void sketch(lzqgram_t& c, const qgram_t pattern) {
        ...
        if(m_filter_table.size() >= m_filter_size) {
            ...
            if(count > min.count) {
                ...
                m_filter_min.extract_min();
                
                assert(m_filter_table.find(min.key) != m_filter_table.end());
                auto min_filter = m_filter_table.find(min.key)->second;
                const auto delta = min_filter.new_count - min_filter.old_count;
                m_filter_table.erase(min_filter.pattern);
                assert(m_filter_table.find(min.key) == m_filter_table.end());
        ...
        if(enter_into_filter) {
            ...
            if(result.second) {
                m_filter_min.insert(pattern, count);
            } else {
...
public:
    SketchProcessor3() : m_filter_table(m_filter_size), m_filter_num(0), m_sketch(m_sketch_cols, m_sketch_rows) {
    }
        ...
        for(size_t j = 0; j < jmax; j++) {
            ...
            // test if prefix is contained in filter
            {
                ...
                if(e != m_filter_table.end()) {
                    ...
                    m_filter_min.increment(prefix, filter_entry.new_count);
